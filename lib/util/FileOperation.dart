...
class FileUtil {
  ...
  Future<void> writeCounter(String path, String content) async {
  ...
  //将数据内容写入指定文件中
  Future<void> writeToFile(File file, String notes) async {
  ...
  Future<Object> getAllSubFile(String path) async {
    ...
    if (directory.existsSync()) {
      var fileList = directory.list();
      await for (final fileSystemEntity in fileList) {
        num++;
        if (num > 100) {
          //超过100条缓存记录 删除缓存
          await fileSystemEntity.delete();
