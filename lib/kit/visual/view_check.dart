...
import 'package:akit/kit/visual/visual.dart';
import 'package:akit/ui/akit_btn.dart';
import 'package:akit/util/screen_util.dart';
import 'package:akit/widget/dash_decoration.dart';
import 'package:akit/widget/widget_build_chain/widget_build_chain_page.dart';
...
class ViewCheckerKit extends VisualKit {
  ...
  void _show(BuildContext? context, OverlayEntry? entrance) {
    ...
    isShown = true;
    aKitOverlayKey.currentState?.insert(focusEntry, below: entrance);
    aKitOverlayKey.currentState?.insert(infoEntry, below: focusEntry);
    aKitOverlayKey.currentState?.insert(rectEntry, below: infoEntry);
    aKitOverlayKey.currentState?.insert(alignEntry, below: rectEntry);
  }
  ...
  @override
  void tabAction() {
    final AKitBtnState? state = AKitBtn.aKitBtnKey.currentState;
    state?.closeDebugPage();
    show(AKitBtn.aKitBtnKey.currentContext, state?.owner);
  }
}
...
class _ViewCheckerWidgetState extends State<ViewCheckerWidget> {
  ...
  static const String _akitViewCheckGroup = 'akit_view_check-group';

  @override
  void dispose() {
    // ignore: invalid_use_of_protected_member
    WidgetInspectorService.instance.disposeGroup(_akitViewCheckGroup);
    super.dispose();
  }
  ...
  String toInfoString(RenderObjectElement element) {
    ...
    if (kDebugMode) {
      ...
      final id = WidgetInspectorService.instance
          // ignore: invalid_use_of_protected_member
          .toId(element.renderObject.toDiagnosticsNode(), _akitViewCheckGroup);
      if (id == null) {
        return '';
      }
      final String nodeDesc = WidgetInspectorService.instance
          .getSelectedSummaryWidget(id, _akitViewCheckGroup);
  ...
  RenderObjectElement? resolveTree() {
    ...
    final ModalRoute<dynamic>? rootRoute =
        ModalRoute.of<dynamic>(AKitApp.appKey.currentContext!);
    ...
    AKitApp.appKey.currentContext?.visitChildElements(filter);
