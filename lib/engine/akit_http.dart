...
import 'package:akit/kit/apm/http_kit.dart';

class AKitHttpOverrides extends HttpOverrides {
  AKitHttpOverrides(this.origin);
  ...
  HttpClient createHttpClient(SecurityContext? context) {
    if (origin != null) {
      return AKitHttpClient(origin!.createHttpClient(context));
    }
    HttpOverrides.global = null;
    final HttpClient client = AKitHttpClient(HttpClient(context: context));
    HttpOverrides.global = this;
    return client;
  }
}

class AKitHttpClient implements HttpClient {
  AKitHttpClient(this.origin);
  ...
  @override
  set keyLog(Function(String line)? callback) => origin.keyLog = callback;
  ...
  Future<HttpClientRequest> monitor(Future<HttpClientRequest> future) async {
    ...
    final HttpKit? kit = ApmKitManager.instance.getKit(ApmKitName.KIT_HTTP);
    kit?.save(httpInfo);
    return AKitHttpClientRequest(request, httpInfo);
  }
...
class AKitHttpClientRequest implements HttpClientRequest {
  AKitHttpClientRequest(this.origin, this.httpInfo);
  ...
  Future<HttpClientResponse> monitor(Future<HttpClientResponse> future) async {
    ...
    return AKitHttpClientResponse(response, recordResponse);
  }
...
class AKitHttpClientResponse implements HttpClientResponse {
  AKitHttpClientResponse(this.origin, this.recordResponse);
